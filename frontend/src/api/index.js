...
const API_BASE = import.meta.env.VITE_API_BASE || 'http://127.0.0.1:8890/api';
const api = axios.create({ baseURL: API_BASE, timeout: 60000 });
export async function getModelConfig(provider) {
    const res = await api.post('/model_providers/get', { provider });
    return res.data;
}
export async function updateModelConfig(provider, values) {
...
export async function updateNovel(payload) {
    const res = await api.post('/novel/update', payload);
    return res.data;
}
export async function deleteNovels(uids) {
    const res = await api.post('/novel/delete', { uids });
    return res.data;
}
export async function listChapters(novel_uid, page = 1, size = 200) {
    const res = await api.post('/chapter/list', { novel_uid, page, size });
    return res.data;
}
export async function getChapter(uid) {
    const res = await api.post('/chapter/get', { uid });
    return res.data;
}
export async function listCharacters(novel_uid, page = 1, size = 200) {
...
export async function updateChapter(payload) {
    const res = await api.post('/chapter/update', payload);
    return res.data;
}
export async function deleteChapters(uids) {
...
export async function deleteCharacters(uids) {
...
export async function ssePost(path, body, onMessage, onDone, options) {
